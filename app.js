...
//  'access-token',
  ...
  document.getElementById("tokeninputfield").innerHTML = ""; 
});

// FUNCION BOTON VERDE

function video(){
  if (document.getElementById('constraints-audio').checked){
...
document.getElementById('camara').addEventListener('click', () => {
  console.log('presionando camara');
});

document.getElementById('microfono').addEventListener('click', () => {
  console.log('presionando microfono');
});

document.getElementById('colgar').addEventListener('click', () => {
  console.log('presionando colgar');

});

document.getElementById('dialer2').addEventListener('click', () => {
  console.log('presionando dialer2');
});
  ...
  return new Promise((resolve) => {
    if (!webex) {
      ...
      webex = window.webex = Webex.init({
        ...
        credentials: {
          access_token: 'ZThmNThmMjgtMmRmNy00YjYxLWFjYTgtZGM3YTNkNTcxYjUzNzkwZTk4YWMtOTBi_PF84_22c4a84e-1b8a-4ad1-b8f8-647e759e4367'
        }
      });
    }
    ...
    memberIDs.forEach((memberID) => {
      ...
      if (memberObject.isUser) {
        if (memberObject.isSelf) {
          console.log('status local' + memberObject.status) ;
        }
        else {
          console.log('status Remote' + memberObject.status) ;;
        }
      }
    });
  });
...
document.getElementById('greenlogin').addEventListener('click', (event) => {
...
document.getElementById('dialer2').addEventListener('click', (event) => {
  // again, we don't want to reload when we try to dial
  event.preventDefault();

  const destination = document.getElementById('tokeninputfield').value;
